...
const T = {
  context: IdioContext,
  async 'works without decompression'({
    start, getFixtureUrl, fixtureName, readFixture,
  }) {
    await start()
  ...
  async 'decompresses json data'({
    start, getFixtureUrl, fixtureJsonName, readJsonFixture,
  }) {
    await start({
      compress: { use: true },
    })
